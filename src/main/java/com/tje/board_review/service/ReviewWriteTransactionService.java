...
@Service
public class ReviewWriteTransactionService {
	@Autowired
	...
	public boolean service(Board_Review b_r, Review_Map r_m) throws Exception {
		...
		int key = (Integer)board_ReviewDAO.insertReturnsKey(b_r);
		board_result = true;
		r_m.setBoard_id(key);
		// board_result = (Integer)board_ReviewDAO.insertReturnsKey(b_r) ==  ? true : false;
		map_result = (Integer)review_MapDAO.insert(r_m) == 1 ? true : false;
